...
require_once ( dirname(__FILE__) . '/templates' . "/template.php"); 
 require_once ( dirname(__FILE__) . '/templates' . "/template2.php"); 
class WD_Portfolio {
	...
	public function wd_portfolio_section_options() {
		...
		<div class="wd-portfolio-meta-section">
			<div class="form-wrap">
				...
				<div class="form-field">
					<label for="wd_portfolio_client"><?php _e('Client', 'wd_portfolio_context')?></label>
					<input type="text" name="wd_portfolio_client" value="<?php echo htmlspecialchars($this->wd_portfolio_get_meta('wd-portfolio-client')); ?>" />
				</div> 
				
				<div class="form-field">
					<label for="wd_portfolio_agency"><?php _e('Agency', 'wd_portfolio_context')?></label>
					<input type="text" name="wd_portfolio_agency" value="<?php echo htmlspecialchars($this->wd_portfolio_get_meta('wd-portfolio-agency')); ?>" />
				</div>
				<div class="form-field">
					<label for="wd_portfolio_style"><?php _e('Choose Style', 'wd_portfolio_context')?></label>
					<input type="radio" name="wd_portfolio_style" value="style1"  checked />Style 1<br>
					<input type="radio" name="wd_portfolio_style" value="style2" <?php checked( htmlspecialchars($this->wd_portfolio_get_meta('wd-portfolio-style')), 'style2' ) ?> />Style 2<br>
				</div> 
				<div class="form-field">
					<label for="wd_portfolio_checkbox"><?php _e('Choose', 'wd_portfolio_context')?></label>
					<input type="radio" name="wd_portfolio_checkbox" value="image" <?php checked( htmlspecialchars($this->wd_portfolio_get_meta('wd-portfolio-checkbox')), 'image' ) ?> />Image<br>
					<input type="radio" name="wd_portfolio_checkbox" value="slider" <?php checked( htmlspecialchars($this->wd_portfolio_get_meta('wd-portfolio-checkbox')), 'slider' ) ?> />Slider<br>
				</div> 				
				<?php
	...
	public function wd_portfolio_save_data($post_id, $post) {
		...
		if( isset($_POST) && isset($_POST['wd_portfolio']) && isset($_POST['wd_portfolio_url']) ) {
			...
			$mydata['wd-portfolio-client'] = $_POST['wd_portfolio_client'];
			$mydata['wd-portfolio-agency'] = $_POST['wd_portfolio_agency'];
			
			$mydata['wd-portfolio-checkbox'] = ( isset( $_POST['wd_portfolio_checkbox'] ) ? sanitize_html_class( $_POST['wd_portfolio_checkbox'] ) : 'style1' );
			
			$mydata['wd-portfolio-style'] = ( isset( $_POST['wd_portfolio_style'] ) ? sanitize_html_class( $_POST['wd_portfolio_style'] ) : '' );
	...
	public function wd_portfolio_show($atts = array()) {
		...
		show_wd_portfolio(  $columns,$show_filter,/*$style,*/$show_title,$show_desc,$count,$portf_style,$show_pages);	
	}


	protected function init_trigger(){
		add_image_size('portfolio_image',480,320,true); 
	}
